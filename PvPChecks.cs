...
namespace PvPChecks
{
    ...
    public class PvPChecks : TerrariaPlugin
    {
        ...
        private async void OnReload(ReloadEventArgs args) // ðåëîàä @iBelarus
        {
            cfg = new ConfigFile<Config>();
            cfg.Read(configPath, out bool write);
            if (write)
        ...
        private void OnPlayerCommand(PlayerCommandEventArgs args) // Ïåðåí¸ñ ñ Essentials+ ñþäà çàïðåò êîìàíä â ïâï
        {
        ...
        private void OnPlayerUpdate(object sender, GetDataHandlers.PlayerUpdateEventArgs args)
        {
            ...
            {
                //player.SetBuff(36, 180); // Îñëàáëåíèå ñîëíå÷íîé áðîíè @iBelarus
                player.SetBuff(Terraria.ID.BuffID.Ichor, 360); // Îñëàáëåíèå ñîëíå÷íîé áðîíè - 195 buff @iBelarus
            }
        ...
        private void OnTogglePvp(object? sender, GetDataHandlers.TogglePvpEventArgs args)
        {
            ...
            {
                args.Player.SendErrorMessage("Solar Flare Armor ([i:2763][i:2764][i:2765]) applies a debuff Withered Armor (Defense is cut in half)."); // ñîîáùåíèå î äåáàôôå ñîëíå÷íîé áðîíè @iBelarus
            }
        }
        private void OnNewProjectile(object sender, GetDataHandlers.NewProjectileEventArgs args)
        {
            ...
            if (tsplayer.TPlayer.hostile && !tsplayer.HasPermission("pvpchecks.ignore"))
            {
                ...
                if (PortalGun && (args.Type == Terraria.ID.ProjectileID.PortalGunBolt || args.Type == Terraria.ID.ProjectileID.PortalGunGate)) // óáèðàòü ñíàðÿäû ïîðòàë ãàíà @iBelarus
                {
                    args.Player.RemoveProjectile(args.Identity, args.Owner);
                    args.Handled = true;
                }
        ...
        private void OnSendData(SendDataEventArgs args)
        {
            ...
            {
                if (TShock.Players[args.number].TPlayer.hostile && args.number2 != 149f && args.number2 != 195f) //ýòî óæå áûëî, òèïî çàïðåùàåò áàôôû íàêèäûâàòü êîìàíäîé âî âðåìÿ ïâï. 149 è 195 - ýòî îêàìåíåíèå (îêàìåíåíèå çà çàïðåòêó) è withered armor (SolarArmorDebuff)
                {
                    args.Handled = true;
                }
            }
        ...
        private void OnGetData(GetDataEventArgs args)
        {
            ...
            if (PortalGun && args.MsgID == PacketTypes.PlayerTeleportPortal && TShock.Players[args.Msg.whoAmI].TPlayer.hostile)
            {
                ...
                TShock.Players[args.Msg.whoAmI].SendErrorMessage("Portal Gun is not allowed in PvP."); // Çàïðåò íà òåëåïîðò â ïîðòàëãàíîâñêèå ïîðòàëû @iBelarus
            }
        }
